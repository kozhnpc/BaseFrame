...
//    private MutableLiveData<String> mBaseData = new MutableLiveData<>();
  ...
  /*  private LiveData<String> mRepositoryData = Transformations.switchMap(mKeyword, new Function<String, LiveData<String>>() {
        ...
        public LiveData<String> apply(String input) {
            ...
            return BaseRepository.getData(BaseViewModel.this, input);
        }
    });*/
...
//    }
    ...
    //上面的代码只是演示如何使用 不是正式的逻辑代码

    //**********************  通用变量封装 ***************************//
    //定义一个加载错误的liveData
    //如果发生错误 更新该变量，view层监听该变量，及时在界面更新UI
    public MutableLiveData<String> mError = getErrorLiveData();


    //定义一个空加载的liveData
    //如果发生空白加载 更新该变量，view层监听该变量，及时在界面更新UI
    public MutableLiveData<String> mEmpty = getEmptyLiveData();

    //当发现返回的数据为空时 让子类调用即可
    public void onEmpty() {
//        LogUtils.i("mEmpty = " + mEmpty.getValue());
        mEmpty.setValue("");
    }
    //**********************  通用变量封装  end ***************************//


    //子类自己传入一个 mEmpty mError 否则公用一个变量，会导致所有的页面都监听同一个变化
    public abstract MutableLiveData<String> getErrorLiveData();

    public abstract MutableLiveData<String> getEmptyLiveData();
