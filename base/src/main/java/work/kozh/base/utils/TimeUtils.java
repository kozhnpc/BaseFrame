...
//                    MusicPlayer.exit();
                    ToastUtils.success(UIUtils.getContext(), "时间到啦，已自动停止播放！");
                    for (TextView view : textViews) {
                        view.setText("音乐播放定时关闭");
                    }
                    cancel();
                    mTimer = null;
                }
    ...
    public static void cancelTimely() {
        if (mTimer != null) {
            mTimer.cancel();
            for (TextView view : textViews) {
                view.setText("音乐播放定时关闭");
    ...
    public static String getMonthBegin() {
        String yearMonth = new SimpleDateFormat("yyyy-MM", Locale.CHINA).format(new Date());
        return yearMonth + "-01";
    }
     ...
     * 返回自定义类型的时间字符串
     *
     * @param format 时间格式化的字符串 yyyy-MM-dd-HH-mm-ss
     * @return
     */
    ...
    public static int getDayLast(String endTime) {
        try {
            ...
            long lastTime = new SimpleDateFormat("yyyy-MM-dd", Locale.CHINA).parse(endTime).getTime();
